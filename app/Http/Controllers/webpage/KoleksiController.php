...
class KoleksiController extends Controller
{
   ...
   //   var_dump($request->query('page'));die();
  	...
  	{
    //	$data = Collection::where('noinduk_deposit', '!=', null)->latest();
    /*  $data = DB::table('catalogs')->whereIn('id',function($query){
          $query->select('catalog_id')->from('collections')->where('noinduk_deposit','!=',null);
      })->orderByRaw('precreatedate desc nulls last');*/
     $data = Catalog::whereHas('collections',function($query) {
      $query->where('noinduk_deposit','!=',null);
      });//->orderByRaw('precreatedate desc nulls last');
  /*   $data = Catalog::whereIn('id',function($query){
      $query->select('catalog_id')->from('collections')->where('noinduk_deposit','!=',null)->orderBy('createdate','desc');
     })*/
    	if (request('type') != ""){
